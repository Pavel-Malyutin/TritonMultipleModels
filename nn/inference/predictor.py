...
class Predictor:
    ...
    @torch.no_grad()
    def predict(self, image: ndarray) -> List[Prediction]:

        detection = self._yolo(image, size=settings.YOLO.PREDICT_SIZE)
        df_results = detection.pandas().xyxy[0]

        license_plate_batch = prepare_recognition_input(
            df_results, image, return_torch=True, device=self._device
        )
        transfer = self._stn(license_plate_batch)
        predictions = self._lprn(transfer)
        predictions_array = predictions.cpu().detach().numpy()

        labels, log_likelihood, _ = self._decode_fn(predictions_array, settings.VOCAB.VOCAB)
